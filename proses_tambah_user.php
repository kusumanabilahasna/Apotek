...
if($_POST){
    ...
    if(empty($nama_user)){
        ...
        include "koneksi.php";
        $insert=mysqli_query($conn,"insert into user (nama_user, jk_user, alamat_user, telepon_user, username, password, role) value ('".$nama_user."','".$jk_user."','".$alamat_user."','".$telepon_user."','".$username."','".md5($password)."','".$role."')") or die(mysqli_error($conn));
        if($insert) {
